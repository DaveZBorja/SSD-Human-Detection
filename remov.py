        ...
        for i in range(detections.shape[2]):
            ...
            if confidence > 0.2:
                ...
                if label == "person":
                    ...
                    # Use a separate thread to speak the detected object label, distance, and position
                    tts_thread = threading.Thread(target=speak_async, args=(f"Person detected at {distance:.2f} centimeters to the {position}",))
                    tts_thread.start()

    # If a person was detected in the last detection, draw the bounding box and label
